<?php
namespace Perspective\PreventAddingSameProductToCartPlugin\Plugin\Model\Checkout\Cart;
        ...
        if ($product) {
            ...
            if ($product && $enableProductCartControl)
            {
                    ...
                    if($this->cartSession->getQuote()->hasProductId($product->getId()))
                    {
                        ...
                        throw new \Magento\Framework\Exception\LocalizedException(
                            __("This product is already in the cart. %1 : %2", $product->getName(), $product->getSku())
                        );
                    }
             }
        }
